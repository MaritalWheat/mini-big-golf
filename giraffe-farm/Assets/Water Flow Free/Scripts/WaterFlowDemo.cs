...
public class WaterFlowDemo : MonoBehaviour
{
	...
	void OnGUI ()
	{
		...
		// Show version number
		GUI.Window(1, new Rect((Screen.width-260), 5, 0, 0), AppNameWindow, "Water Flow FREE 1.0.2");
		
		// Show Scene GUI window
		int DemoWaterWindowWidth = 10 + (195*transform.childCount);
		GUI.Window(2, new Rect((Screen.width-DemoWaterWindowWidth)/2, Screen.height-65, 0, 0), DemoWater, "Select Water");
